   ...
   <?php include_once("../htm-head.php") ?>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
        <script type="text/javascript" >
            ...
            function back(){
                window.location.href='client_web.php'
            }
...
//    echo "<h3 class='ui-bar ui-bar-a ui-corner-all'>發問時間:".$row['time']."</h3>";
    ...
    if(!empty($row['answer'])){
        ...
        echo "<button type='button' onclick='expend()'>修改答案</button><br>";
        echo "<span id='myid' name='myid' style='display:none;'>
        <textarea cols='30' rows='10' style='resize: none;'  name='answer'>".$row['answer']."</textarea><br>
        <input type='submit' value='送出'>
        </span>";
    }else{
        echo "<button type='button' onclick='expend()'>回覆</button><br>";
        echo "<span id='myid' name='myid' style='display:none;'>
        <textarea cols='30' rows='10' style='resize: none;'  name='answer' onclick='cleandef(this)'>把您想回覆的答案打在這裡：(限255個字)</textarea><br>
        <input type='submit' value='送出'>
        </span>";
    }
